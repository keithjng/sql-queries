...
SELECT billing_state, SUM(total) AS invoice_total
FROM invoices
WHERE billing_country = 'USA'
GROUP BY billing_state
...
SELECT billing_state, AVG(total) AS invoice_average
FROM invoices
WHERE billing_country = 'USA'
GROUP BY billing_state
...
SELECT billing_city, AVG(total) AS invoice_average
FROM invoices
WHERE billing_country = 'USA'
GROUP BY billing_city
ORDER BY invoice_average DESC
...
SELECT billing_city, COUNT(*) AS invoice_count
FROM invoices
WHERE billing_state = 'CA' AND billing_country = 'USA'
GROUP BY billing_city
ORDER BY invoice_count DESC
LIMIT 3;
...
SELECT country, COUNT(DISTINCT id) AS customer_total
FROM customers
GROUP BY country
...
SELECT city, COUNT(DISTINCT id) AS customer_total
FROM customers
GROUP BY city
ORDER BY customer_total DESC
LIMIT 7;
