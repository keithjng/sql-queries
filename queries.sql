...
SELECT * FROM invoices WHERE billing_country IN ('USA') ORDER BY total DESC LIMIT 10;

-- The 10 least expensive invoices from the US
SELECT * FROM invoices WHERE billing_country IN ('USA') ORDER BY total LIMIT 10;

-- The 10 most expensive invoices from outside the US
-- Hint: If "=" means equal, use "!=" to mean "not equal"
SELECT * FROM invoices WHERE billing_country NOT IN ('USA') ORDER BY total DESC LIMIT 10;
...
SELECT billing_state, COUNT(*), SUM(total), AVG(total) FROM invoices WHERE billing_country = 'USA' GROUP BY billing_state ORDER BY avg DESC;
...
SELECT billing_country, SUM(total) FROM invoices GROUP BY billing_country ORDER BY sum DESC LIMIT 5;

-- A list of the top 5 countries by average invoice size
SELECT billing_country, AVG(total) FROM invoices GROUP BY billing_country ORDER BY avg DESC LIMIT 5;
